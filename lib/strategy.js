...
OAuth2Strategy.prototype.authenticate = function(req, options) {
  ...
  if (req.query && req.query.code) {
    function loaded(err, ok, state) {
      ...
      if (req.session && req.session[self._key] && req.session[self._key].customState) {
        req.query.state = req.session[self._key].customState;
        delete req.session[self._key].customState;
      ...
      function stored(err, state) {
        ...
        if (state) {
          ...
          if (options.customState) {
            req.session[self._key].customState = options.customState;
          }
        }
